...
int main(void)
{
    ...
    inorder(rbt, rbt->r, 0);
    stack = (int*)malloc(sizeof(int)*(total + 1));
    top = 0;
    inorder(rbt, rbt->r, 1);
    /*
     printf("filename=%s\n", fn);
    ...
    while( !feof(in)){
        ...
        for( i = 0 ; i < top ; i ++ ){
            if( stack[i] == num ){
                if( i == 0 ) fprintf(out, "NIL ");
                else fprintf(out, "%d ", stack[i-1]);
                
                fprintf(out, "%d ", stack[i]);
                
                if( i < top - 1 ) fprintf(out, "%d", stack[i+1]);
                else fprintf(out, "NIL");
                fprintf(out, "\n");
            }else if( i == 0 && num < stack[i] ){
                fprintf(out, "NIL NIL %d\n", stack[i]);
            }
            else if( i < top-1 && stack[i] < num && num < stack[i+1] ){
                fprintf(out, "%d NIL %d\n", stack[i], stack[i+1]);
            } else if( i == top - 1 && stack[i] < num ){
                fprintf(out, "%d NIL NIL\n", stack[i]);
            }
        }
    }
    fclose(in);
